...
<table class="table table-sm table-light-header">
	...
	<tbody>
		...
		foreach($tasks as $task){
			...
			if(GetCreativesCount($pdo, $task['task_id'], $user_id)['user'] > 0){
				...
				echo "<td><span id='MyCountSrc{$task['task_id']}'>Мои креативы: <span class='badge badge-primary badge-pill'>".GetCreativesCount($pdo, $task['task_id'], $user_id)['user']."</span> / Всего в задаче: <span class='badge badge-primary badge-pill'>".GetCreativesCount($pdo, $task['task_id'], $user_id)['all']."</span></td>";			
			}	
			echo "<td>";
